...
type ProductInfoCardProps = {
  ...
  sizes?: string;
  standards?: string;
  onReadMore?: (e: MouseEvent<HTMLButtonElement>) => void;
};

export function ProductInfoCard({ title, imageUrl, summary, sizes, standards, onReadMore }: ProductInfoCardProps) {
  const firstStandard = (standards || '').split(/[–,|/]/)[0]?.trim();
  return (
    <div className="bg-[#E5E8EB] shadow-[0_4px_4px_rgba(0,0,0,0.1)] h-full flex flex-col">
      <div className=" w-full overflow-hidden bg-[#E9EDF0]">
      ...
      <div className="p-6 flex-1 flex flex-col gap-4">
        ...
        <div className="pt-2 mt-auto">
          ...
          >
            Read More <img src="/images/services/red-arrow.svg" alt="Arrow" className="w-3 h-3" />
          </button>
        </div>
      </div>
    </div>
